...
static int get_string_property(IOHIDDeviceRef device, CFStringRef prop, wchar_t *buf, size_t len)
{
	...
	if (str) {
		...
		range.location = 0;
		range.length = (CFIndex) (((size_t)str_len > len)? len: (size_t)str_len);
...
struct hid_device_info  HID_API_EXPORT *hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
	...
	IOHIDDeviceRef *device_array = (IOHIDDeviceRef *) calloc((size_t) num_devices, sizeof(IOHIDDeviceRef));
                         ...
                         uint8_t *report, CFIndex report_length)
{
	...
	rpt->data = (uint8_t *) calloc(1, (size_t) report_length);
	memcpy(rpt->data, report, (size_t) report_length);
...
//        goto return_error;
	...
	if (ret == kIOReturnSuccess) {
		...
		dev->input_report_buf = (uint8_t *) calloc((size_t) dev->max_input_report_len, sizeof(uint8_t));
...
// REPLACED GOTOS from original hidapi because they're gross
//return_error:
//    if (dev->device_handle != NULL)
//        CFRelease(dev->device_handle);
//
